using Microsoft.VisualStudio.TestTools.UnitTesting;
using QuantityMeasurementMain.ModelLayer;

namespace QuantityMeasurementTest
{
    [TestClass]
    [TestCategory("AdditionOfLength")]
    public class LengthAdditionMsTest
    {
        private const double EPSILON = 0.0001;

        [TestMethod]
        public void TestAddition_SameUnit_FeetPlusFeet()
        {
            var q1 = new Length(1.0, Length.LengthUnit.FEET);
            var q2 = new Length(2.0, Length.LengthUnit.FEET);

            var result = q1.Add(q2);

            Assert.AreEqual(3.0, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.FEET, result.Unit);
        }

        [TestMethod]
        public void TestAddition_SameUnit_InchPlusInch()
        {
            var q1 = new Length(6.0, Length.LengthUnit.INCHES);
            var q2 = new Length(6.0, Length.LengthUnit.INCHES);

            var result = q1.Add(q2);

            Assert.AreEqual(12.0, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.INCHES, result.Unit);
        }

        [TestMethod]
        public void TestAddition_CrossUnit_FeetPlusInches()
        {
            var q1 = new Length(1.0, Length.LengthUnit.FEET);
            var q2 = new Length(12.0, Length.LengthUnit.INCHES);

            var result = q1.Add(q2);

            Assert.AreEqual(2.0, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.FEET, result.Unit);
        }

        [TestMethod]
        public void TestAddition_CrossUnit_InchPlusFeet()
        {
            var q1 = new Length(12.0, Length.LengthUnit.INCHES);
            var q2 = new Length(1.0, Length.LengthUnit.FEET);

            var result = q1.Add(q2);

            Assert.AreEqual(24.0, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.INCHES, result.Unit);
        }

        [TestMethod]
        public void TestAddition_CrossUnit_YardPlusFeet()
        {
            var q1 = new Length(1.0, Length.LengthUnit.YARDS);
            var q2 = new Length(3.0, Length.LengthUnit.FEET);

            var result = q1.Add(q2);

            Assert.AreEqual(2.0, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.YARDS, result.Unit);
        }

        [TestMethod]
        public void TestAddition_CrossUnit_CentimeterPlusInch()
        {
            var q1 = new Length(2.54, Length.LengthUnit.CENTIMETER);
            var q2 = new Length(1.0, Length.LengthUnit.INCHES);

            var result = q1.Add(q2);

            Assert.AreEqual(5.08, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.CENTIMETER, result.Unit);
        }

        [TestMethod]
        public void TestAddition_Commutativity()
        {
            var a = new Length(1.0, Length.LengthUnit.FEET);
            var b = new Length(12.0, Length.LengthUnit.INCHES);

            var result1 = a.Add(b);
            var result2 = b.Add(a);

            Assert.IsTrue(result1.Equals(result2));
        }

        [TestMethod]
        public void TestAddition_WithZero()
        {
            var q1 = new Length(5.0, Length.LengthUnit.FEET);
            var q2 = new Length(0.0, Length.LengthUnit.INCHES);

            var result = q1.Add(q2);

            Assert.AreEqual(5.0, result.Value, EPSILON);
            Assert.AreEqual(Length.LengthUnit.FEET, result.Unit);
        }

        [TestMethod]
        public void TestAddition_NegativeValues()
        {
            var q1 = new Length(5.0, Length.LengthUnit.FEET);
            var q2 = new Length(-2.0, Length.LengthUnit.FEET);

            var result = q1.Add(q2);

            Assert.AreEqual(3.0, result.Value, EPSILON);
        }

        [TestMethod]
        public void TestAddition_LargeValues()
        {
            var q1 = new Length(1e6, Length.LengthUnit.FEET);
            var q2 = new Length(1e6, Length.LengthUnit.FEET);

            var result = q1.Add(q2);

            Assert.AreEqual(2e6, result.Value, EPSILON);
        }

        [TestMethod]
        public void TestAddition_SmallValues()
        {
            var q1 = new Length(0.001, Length.LengthUnit.FEET);
            var q2 = new Length(0.002, Length.LengthUnit.FEET);

            var result = q1.Add(q2);

            Assert.AreEqual(0.003, result.Value, EPSILON);
        }
    }
}