...
using QMAOperationService.Exceptions;
using QMAOperationService.Interfaces;

namespace QMAOperationService.Services
{
    /// <summary>
    /// Handles all quantity measurement computations (stateless).
    /// Persists operation history by calling HistoryService over HTTP.
    /// </summary>
    public class QuantityMeasurementService : IQuantityMeasurementService
    {
        ...
        public async Task<bool> Compare(QuantityDTO first, QuantityDTO second, string? userId = null)
        {
            ...
            await SaveOperationAsync(first, second, "Compare", result ? 1 : 0, "Boolean", userId);
        ...
        public QuantityDTO Convert(QuantityDTO source, string targetUnit)
        {
            ...
            // Convert is stateless — no history entry needed (no userId context)
            return new QuantityDTO { Value = convertedValue, Unit = targetUnit, Category = source.Category };
        }

        public async Task<QuantityDTO> Add(QuantityDTO first, QuantityDTO second, string targetUnit, string? userId = null)
        {
            var result = ApplyArithmetic(first, second, "addition", (a, b) => a + b, targetUnit);
            await SaveOperationAsync(first, second, "Addition", result.Value, result.Unit, userId);
            return result;
        }

        public async Task<QuantityDTO> Subtract(QuantityDTO first, QuantityDTO second, string targetUnit, string? userId = null)
        {
            var result = ApplyArithmetic(first, second, "subtraction", (a, b) => a - b, targetUnit);
            await SaveOperationAsync(first, second, "Subtraction", result.Value, result.Unit, userId);
            return result;
        }

        public async Task<double> Divide(QuantityDTO first, QuantityDTO second, string? userId = null)
        {
            Validate(first); Validate(second);
            ...
            double result = a / b;
            await SaveOperationAsync(first, second, "Division", result, "Ratio", userId);
            return result;
        }

        // ─── ARITHMETIC HELPER ────────────────────────────────────────────

        private QuantityDTO ApplyArithmetic(
            QuantityDTO first, QuantityDTO second,
            string operationName, Func<double, double, double> operation,
            string? targetUnit = null)
        {
            ...
            if (!first.Unit.Equals(second.Unit, StringComparison.OrdinalIgnoreCase))
                throw new QuantityMeasurementException(
                    $"Units must match when no target unit is provided. Got '{first.Unit}' and '{second.Unit}'.");

            return new QuantityDTO
            {
                Value = operation(first.Value, second.Value),
                Unit = first.Unit,
        ...
        private void EnsureSameCategory(QuantityDTO first, QuantityDTO second, string operation)
        {
            if (!first.Category.Equals(second.Category, StringComparison.OrdinalIgnoreCase))
                throw new QuantityMeasurementException(
                    $"Cannot {operation} quantities of different categories ('{first.Category}' vs '{second.Category}').");
        }
        ...
        // ─── INTER-SERVICE CALL TO HISTORY SERVICE ────────────────────────
            ...
            string? userId)
        {
            try
            {
                ...
                var payload = new SaveOperationRequest
                {
                    First        = first,
                    Second       = second,
                ...
                var response = await client.PostAsync("/api/history/save", content);

                if (!response.IsSuccessStatusCode)
                    _logger.LogWarning(
                        "HistoryService returned {StatusCode} for operation {Op}",
                        response.StatusCode, operationType);
            }
            catch (Exception ex)
            {
                // History saving is non-critical; log but don't fail the operation
                _logger.LogError(ex, "Failed to save operation history for {Op}", operationType);
            }
        }
    }
}
